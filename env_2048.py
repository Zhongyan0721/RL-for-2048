...
class Game2048Env(gym.Env):
    ...
    def step(self, action):
        ...
        if not moved:
            ...
            # Reward 1: Log score (stabilizes gradients)
            if score_gain > 0:
                reward += np.log2(score_gain)
            
            # Reward 2: Small bonus for empty cells (encourages space management)
            n_empty = len(np.where(self.grid == 0)[0])
            reward += n_empty * 0.1
